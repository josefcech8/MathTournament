/*
    IMPORTANT
    (1) Add teams and their passwords
    (2) Add path to every fileHandler, every picture
    (3) Add login files to every team
    (4) ban na naboj.org
    (5) Vytvořit universal user kdyby se něco posralo

    TODO
    (1) Make components in initTestPanel private
    (2)
    (3)
    (4)
    (5) update cleancode in FileHandler.java
    (6)
    (7) highlight your team in the table
    (8) put <sup> or <sub> and <sup>1</sup>&frasl;<sub>10</sub> tags in taskContent and in units
    (9) Blbuvzodrnost u zadávání odpovědí
        disable submit result with empty or incorrect textResult
    (10)
    (11) sjednotit barvy tlačítek
    (12) create table for winners
 */

public class MathTournament {
    public static void main(String[] args) {
        new GraphicalUserInterface();
    }
}